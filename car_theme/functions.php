...
function query_car($query)
{
	$args = array(
		...
		'order' => 'DESC',
		// 'posts_per_page' => 1,
		//Giá bắt đầu - Giá kết thúc, Người bán
		'meta_query' => array(
			array(
					'key'   => 'start_price',
					'value' => $start_price,
		...
		'tag_slug__and' => array($city, $district, $year), //Thành phố , Quận, Năm sản xuất
		'category__and' => array( 1, 2 ) //Hãng xe, mẫu xe
	);
	//keyword query
	$keyword = $_GET["keyword"];
	$age_filter = function ($where = '') use ($options) {
    $where .= " AND (post_title LIKE '%" . $keyword . "%' AND post_status = 'publish')";
    return $where;
	};
